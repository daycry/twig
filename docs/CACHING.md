...
Implemented via `CICacheAdapter` when the cache service is available. Each
compiled template is stored as a signed JSON payload:
```json
{ "t": <unix_timestamp>, "c": "<?php class ... >", "s": "<hex hmac sha256>" }
```
Key format: `<prefix><sha256(className::logicalName)>`.

An index key `<prefix>__index` tracks all compiled keys for full clear
operations. Prefix is configurable; see section 6.

#### HMAC payload signing

Because the adapter's `load()` step ends in `eval('?>' . $data['c'])`, the
adapter signs every payload it writes with HMAC-SHA256 over `c`. On
read-back the signature is recomputed and compared with `hash_equals()`
*before* the eval; a missing or invalid signature causes the entry to be
discarded (and best-effort deleted so the next render recompiles), with
`event=twig.cache.adapter.signature_invalid` logged at warning level.

The HMAC key is derived in this order:
1. `Config\Encryption::$key` if set (typed as `string`, non-empty).
2. Otherwise a stable per-install fallback: `hash('sha256', WRITEPATH . APPPATH)`.

The fallback is **not a secret** — it only protects against accidental
cross-environment cache reuse. For real adversarial protection (shared
Redis with multiple tenants, untrusted network, etc.) configure the
framework's encryption key. See `docs/TROUBLESHOOTING.md` →
*"Cache writes succeed but reads always recompile"* for the typical symptom
when an upgrade ships unsigned legacy entries.
