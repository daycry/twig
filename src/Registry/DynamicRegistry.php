...
class DynamicRegistry
{
    ...
    public function registerFunction(string $name, callable $callable, array $options, ?Environment $env, bool $envReady): void
    {
        if ($env !== null && $envReady) {
            ...
            if (function_exists('log_message')) { log_message('info','event=twig.function.registered name='.$name); }
            return;
        }
        ...
        if (function_exists('log_message')) { log_message('info','event=twig.function.queued name='.$name); }
    }
    ...
    public function registerFilter(string $name, callable $callable, array $options, ?Environment $env, bool $envReady): void
    {
        if ($env !== null && $envReady) {
            ...
            if (function_exists('log_message')) { log_message('info','event=twig.filter.registered name='.$name); }
            return;
        }
        $this->pendingFilters[] = [
            'name' => $name,
        ...
        if (function_exists('log_message')) { log_message('info','event=twig.filter.queued name='.$name); }
    }
    ...
    {
        $removed = false;
        if (isset($this->functions[$name])) { unset($this->functions[$name]); $removed = true; }
        if ($this->pendingFunctions) {
            $this->pendingFunctions = array_values(array_filter($this->pendingFunctions, static fn($f) => $f['name'] !== $name));
        }
        if ($removed && function_exists('log_message')) { log_message('info','event=twig.function.unregistered name='.$name); }
        return $removed;
    }
    ...
    {
        $removed = false;
        if (isset($this->filters[$name])) { unset($this->filters[$name]); $removed = true; }
        if ($this->pendingFilters) {
            $this->pendingFilters = array_values(array_filter($this->pendingFilters, static fn($f) => $f['name'] !== $name));
        }
        if ($removed && function_exists('log_message')) { log_message('info','event=twig.filter.unregistered name='.$name); }
        return $removed;
    }
}
